...
class bc_LSTM:
	...
	def load_data(self,):
		...
		self.train_x = self.data.train_text_x,self.data.train_audio_x
		self.val_x = self.data.val_text_x, self.data.val_audio_x
		self.test_x = self.data.test_text_x, self.data.test_text_x
	...
	def get_bimodal_model(self):
		...
		inputs =[train_text_emb['0'],train_audio_emb['0']]
		masked = Masking(mask_value =0)(inputs)
		post = tf.keras.layers.Dense(units=10)
		output =post(embraced_output)
